...
@SuppressWarnings("unused") public class Block extends GeneratableElement
	...
	@Override public BufferedImage generateModElementPicture() {
		...
		} else if (blockBase != null && blockBase.equals("Chain")) {
			if (itemTexture.isEmpty()) {
				return (BufferedImage) MinecraftImageGenerator.Preview.generateChainIcon(getMainTexture());
			} else {
				return ImageUtils.resizeAndCrop(
						getModElement().getFolderManager().getTextureImageIcon(itemTexture, TextureType.ITEM)
								.getImage(), 32);
			}
