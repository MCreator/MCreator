...
public class FluidSpecialInformationConverter implements IConverter {

	@Override
	public GeneratableElement convert(Workspace workspace, GeneratableElement input, JsonElement jsonElementInput) {
		Fluid fluid = (Fluid) input;
		JsonObject oldFluid = jsonElementInput.getAsJsonObject().getAsJsonObject("definition");

		List<String> specialInfo = new ArrayList<>();
		if (oldFluid.get("specialInfo") != null)
			oldFluid.getAsJsonArray("specialInfo").iterator()
					.forEachRemaining(jsonElement -> specialInfo.add(jsonElement.getAsString()));

		fluid.specialInformation.setFixedText(
