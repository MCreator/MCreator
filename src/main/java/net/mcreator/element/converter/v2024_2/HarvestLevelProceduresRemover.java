...
public class HarvestLevelProceduresRemover extends ProcedureConverter {
	...
	private static final String PLACEHOLDER_BLOCK = """
			<block type="math_number">
				...
				<comment h="92" w="220">Harvest level system no longer exists so this block was automatically replaced with placeholder value of 0</comment>
			</block>
			""";

	private static final List<String> HARVEST_LEVEL_BLOCKS = Arrays.asList("blockat_harvestlevel",
			"item_get_harvest_level");
	...
	@Override protected String fixXML(Procedure procedure, String xml) throws Exception {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (HARVEST_LEVEL_BLOCKS.contains(element.getAttribute("type"))) {
