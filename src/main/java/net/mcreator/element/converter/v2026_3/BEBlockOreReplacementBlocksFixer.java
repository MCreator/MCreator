...
/**
 * Blocks.STONE wildcard was the default ore replacement block and was removed from the Bedrock Edition mappings.
 * This converter replaces it with the current default set of ore replacement blocks.
 */
public class BEBlockOreReplacementBlocksFixer implements IConverter {

	@Override
	public GeneratableElement convert(Workspace workspace, GeneratableElement input, JsonElement jsonElementInput) {
		BEBlock block = (BEBlock) input;
		...
		if (block.blocksToReplace != null) {
			...
			if (noDuplicatesList.contains(STONE)) {
				...
				noDuplicatesList.addAll(List.of(
						...
						new MItemBlock(workspace, "Blocks.TUFF"),
						new MItemBlock(workspace, "Blocks.DEEPSLATE")
						//@formatter:on
				));
			}
