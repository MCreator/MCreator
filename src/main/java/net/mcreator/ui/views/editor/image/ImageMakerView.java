...
public class ImageMakerView extends ViewBase implements MouseListener, MouseMotionListener {
	...
	public void saveAs() {
		...
		if (namec != null) {
			File exportFile;
			if (n == 0)
				exportFile = mcreator.getFolderManager().getTextureFileFromSectionType(RegistryNameFixer.fix(namec),
						TextureSection.BLOCK);
			else if (n == 1)
				exportFile = mcreator.getFolderManager().getTextureFileFromSectionType(RegistryNameFixer.fix(namec), TextureSection.ITEM);
			else if (n == 2)
				exportFile = mcreator.getFolderManager().getTextureFileFromSectionType(RegistryNameFixer.fix(namec), TextureSection.OTHER);
			else
				return;
