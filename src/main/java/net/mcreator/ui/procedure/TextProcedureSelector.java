...
public class TextProcedureSelector extends AbstractProcedureSelector {

	@Nullable private final JTextField fixedText;
	...
	public TextProcedureSelector(@Nullable IHelpContext helpContext, MCreator mcreator, @Nullable JTextField fixedText,
		...
		this(helpContext, mcreator, L10N.t("elementgui.common.value"), Side.BOTH, fixedText, 50, providedDependencies);
	}
	...
	public TextProcedureSelector(@Nullable IHelpContext helpContext, MCreator mcreator, String eventName, Side side,
			@Nullable JTextField fixedText, int width, Dependency... providedDependencies) {
		super(mcreator, VariableTypeLoader.BuiltInTypes.STRING, providedDependencies);
		...
		if (fixedText != null)
			add("East", PanelUtils.totalCenterInPanel(
					PanelUtils.join(FlowLayout.LEFT, 0, 1, PanelUtils.westAndEastElement(procedures, fixedText),
		...
		if (fixedText != null)
			fixedText.setPreferredSize(new Dimension(width, 0));
	...
	@Override protected CBoxEntry updateDepsList(boolean smallIcons) {
		...
		if (fixedText != null)
			fixedText.setEnabled(!edit.isEnabled());
	...
	@Override public TextProcedure getSelectedProcedure() {
		...
		if (fixedText != null) {
			Object rawValue = fixedText.getText();
			if (rawValue != null)
		...
		if (selected == null || selected.string.equals(defaultName))
			return new TextProcedure(null, value);
		return new TextProcedure(selected.string, value);
	}

	public void setSelectedProcedure(TextProcedure procedure) {
		if (procedure != null) {
			if (procedure.getName() != null)
				procedures.setSelectedItem(new CBoxEntry(procedure.getName(), null));

			if (fixedText != null)
				fixedText.setText(procedure.getFixedText());
		}
	}
}
