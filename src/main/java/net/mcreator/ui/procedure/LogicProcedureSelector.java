...
public class LogicProcedureSelector extends AbstractProcedureSelector {
	...
	@Override public LogicProcedure getSelectedProcedure() {
		...
		if (fixedValue != null)
			value = fixedValue.isSelected();

		CBoxEntry selected = procedures.getSelectedItem();
		if (selected == null || selected.string.equals(defaultName))
			return new LogicProcedure(null, value);
		return new LogicProcedure(selected.string, value);
	}

	public void setSelectedProcedure(LogicProcedure procedure) {
		if (procedure != null) {
			if (procedure.getName() != null)
				procedures.setSelectedItem(new CBoxEntry(procedure.getName(), null));

			if (fixedValue != null)
				fixedValue.setSelected(procedure.getFixedValue());
		}
	}
