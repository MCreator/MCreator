...
public class ImageDialog extends AbstractWYSIWYGDialog {
	...
	public ImageDialog(WYSIWYGEditor editor, @Nullable Image image) {
		...
		VComboBox<String> textureSelector = new SearchableComboBox<>(
				editor.mcreator.getFolderManager().getTexturesListFromSectionType(WorkspacePanelTextures.Section.OTHER).stream().map(File::getName)
						.toArray(String[]::new));
			...
			if (imageTxt != null) {
				if (image == null) {
					ImageIcon a = new ImageIcon(editor.mcreator.getFolderManager()
							.getTextureFileTypeFromID(FilenameUtilsPatched.removeExtension(imageTxt), WorkspacePanelTextures.Section.OTHER).getAbsolutePath());
