...
public class AIConditionEditor {
	...
	public static List<String> open(MCreator parent, @Nullable String[] data) {
		ProcedureSelector startCondition = new ProcedureSelector(
				IHelpContext.NONE.withEntry("entity/ai_start_condition"), parent, "Additional start condition",
		...
		ProcedureSelector continueCondition = new ProcedureSelector(
				IHelpContext.NONE.withEntry("entity/ai_continue_condition"), parent, "Additional continue condition",
				ProcedureSelector.Side.BOTH, false, VariableElementType.LOGIC,
				Dependency.fromString("x:number/y:number/z:number/world:world/entity:entity"));

		startCondition.setDefaultName("(no additional condition)").refreshList();
		continueCondition.setDefaultName("(no additional condition)").refreshList();
		...
		JDialog dialog = pane.createDialog(parent, "AI condition editor");
