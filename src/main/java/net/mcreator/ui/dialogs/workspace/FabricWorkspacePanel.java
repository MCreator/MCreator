...
public class FabricWorkspacePanel extends AbstractWorkspacePanel {
	...
	public FabricWorkspacePanel(Window parent) {
		...
		add(PanelUtils.westAndEastElement(new JLabel(L10N.t("dialog.new_workspace.fabric.display_name")),
				PanelUtils.join(workspaceDialogPanel.modName)));

		add(new JEmptyBox(5, 5));

		add(PanelUtils.westAndEastElement(new JLabel(L10N.t("dialog.new_workspace.fabric.modid")),
				PanelUtils.join(workspaceDialogPanel.modID)));

		add(new JEmptyBox(5, 5));

		add(PanelUtils.westAndEastElement(new JLabel(L10N.t("dialog.new_workspace.fabric.generator")),
				PanelUtils.join(workspaceDialogPanel.generatorSelector)));

		add(new JEmptyBox(30, 30));

		add(PanelUtils.westAndEastElement(new JLabel(L10N.t("dialog.new_workspace.fabric.package")),
				PanelUtils.join(workspaceDialogPanel.packageName)));

		add(new JEmptyBox(30, 30));

		add(PanelUtils.westAndEastElement(new JLabel(L10N.t("dialog.new_workspace.fabric.folder")),
		...
		add(PanelUtils.join(FlowLayout.LEFT, new JLabel(UIRES.get("18px.info")), new JEmptyBox(0, 0),
				new JLabel(L10N.t("dialog.new_workspace.fabric.notice"))));
