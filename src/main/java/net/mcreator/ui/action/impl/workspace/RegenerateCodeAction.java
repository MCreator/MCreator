...
public class RegenerateCodeAction extends GradleAction {
	...
	public static void regenerateCode(MCreator mcreator, boolean warnLockedCode, boolean warnMissingDefinitions,
		...
		Thread thread = new Thread(() -> {
			...
			// Delete all managed tag entries, except entries owned by code-locked mod elements because
			// code generation is skipped for them, so they would not re-add their entries during regeneration
			Set<String> lockedElements = mcreator.getWorkspace().getModElements().stream()
					.filter(ModElement::isCodeLocked).map(ModElement::getName).collect(Collectors.toSet());
			for (Map.Entry<TagElement, ArrayList<TagElement.Entry>> tag : mcreator.getWorkspace().getTagElements()
					.entrySet()) {
				tag.getValue().removeIf(e -> e.isManaged() && !lockedElements.contains(e.owner()));
			}
