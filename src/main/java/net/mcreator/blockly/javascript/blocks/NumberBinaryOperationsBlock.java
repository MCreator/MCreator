...
public class NumberBinaryOperationsBlock implements IBlockGenerator {
	...
	@Override public void generateBlock(BlocklyToCode master, Element block) throws TemplateGeneratorException {
		...
		if (a != null && b != null && operationType != null) {
			...
			if (operationType.equals("DIVIDE_DOUBLE")) {
				...
				master.append(")");
			} else if (JavaScriptKeywordsMap.BINARY_MATH_OPERATORS.get(operationType) != null) {
				String operator = JavaScriptKeywordsMap.BINARY_MATH_OPERATORS.get(operationType);
				master.append("(");
				master.append(withoutParentheses(codeA, operator));
				master.append(operator);
				master.append(withoutParentheses(codeB, operator));
				master.append(")");
			} else if (JavaScriptKeywordsMap.MATH_METHODS.get(operationType) != null) {
	...
	private static String withoutParentheses(String code, String operator) {
		...
		if (operator.equals("&") || operator.equals("^") || operator.equals("|")) {
			return ProcedureCodeOptimizer.toInt(code, lowerPriority);
		}
