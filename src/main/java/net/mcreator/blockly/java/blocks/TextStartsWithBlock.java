...
public class TextStartsWithBlock implements IBlockGenerator {
	...
	@Override public void generateBlock(BlocklyToCode master, Element block) throws TemplateGeneratorException {
		...
		for (Element element : elements)
			if (element.getNodeName().equals("value"))
				if (element.getAttribute("name").equals("starts"))
					contains = element;
		...
		if (text != null && contains != null) {
			...
			master.append(".startsWith(");
			master.processOutputBlockWithoutParentheses(contains);
			master.append("))");
		} else {
			master.addCompileNote(new BlocklyCompileNote(BlocklyCompileNote.Type.ERROR,
					L10N.t("blockly.errors.empty_text_starts_with")));
		}
	}

	@Override public String[] getSupportedBlocks() {
		return new String[] { "text_starts_with" };
	}
