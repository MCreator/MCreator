...
public class Workspace implements Closeable, IGeneratorProvider {
	...
	@Override public boolean equals(Object o) {
		...
		if (o instanceof Workspace other) {
			try {
				return getWorkspaceFolder().getCanonicalPath().equals(other.getWorkspaceFolder().getCanonicalPath());
			} catch (Exception ignored) {
				return getWorkspaceFolder().getAbsolutePath().equals(other.getWorkspaceFolder().getAbsolutePath());
			}
		}
	...
	@Override public int hashCode() {
		try {
			return getWorkspaceFolder().getCanonicalPath().hashCode();
		} catch (Exception ignored) {
			return getWorkspaceFolder().getAbsolutePath().hashCode();
		}
	}
	...
	public static Workspace readFromFS(File workspaceFile, @Nullable Window ui)
		...
		if (workspaceFile.isFile()) {
			...
			Workspace retval;
			try {
				retval = gson.fromJson(workspace_string, Workspace.class);
	...
	@VisibleForTesting public static Workspace readFromFSUnsafe(File workspaceFile,
		...
		Workspace retval = gson.fromJson(FileIO.readFileToString(workspaceFile), Workspace.class);
		retval.fileManager = new WorkspaceFileManager(workspaceFile, retval);
		retval.userSettingsManager = new WorkspaceUserSettingsManager(retval, retval.getFolderManager());
		retval.historyManager = new HistoryManager(retval);
	...
	public void reloadFromFileSystem() {
		...
		loadStoredDataFrom(fileManager.gson.fromJson(FileIO.readFileToString(workspaceFileToRead), Workspace.class));
