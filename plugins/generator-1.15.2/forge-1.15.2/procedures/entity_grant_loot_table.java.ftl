try {
	if (${input$entity} instanceof ServerPlayerEntity) {
		ServerPlayerEntity _ent = ${input$entity};
		_ent.server.getLootTableManager().getLootTableFromLocation(new ResourceLocation(${input$lootTable})).fillInventory((IInventory) _ent.inventory,
			...
			.withParameter(LootParameters.BLOCK_ENTITY, _ent.getServerWorld().getPendingTileEntityAt(new BlockPos(_ent)).withParameter(LootParameters.TOOL, _ent.inventory.getCurrentItem())
