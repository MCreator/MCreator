...
public class ${name}Model extends AnimatedGeoModel<${name}Entity> {
	...
	public ResourceLocation getModelLocation(TestEntity object) {
		return new ResourceLocation("${modid}:geo/${data.mobModelName}.geo.json");
	}

	@Override
	public ResourceLocation getAnimationFileLocation(TestEntity animatable) {
		return new ResourceLocation("${modid}:animations/${data.mobModelName}.animation.json");
	}
