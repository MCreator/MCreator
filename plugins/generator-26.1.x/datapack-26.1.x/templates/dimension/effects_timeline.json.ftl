<#-- Timeline for custom effects dimensions with NORMAL sky type. It replicates the vanilla
     minecraft:day timeline to provide the overworld day/night cycle. Color tracks use the
     multiply modifier, so custom base colors defined in dimension type attributes are preserved.
     Fog and sunrise/sunset color tracks are only added if sun height is allowed to affect the
     fog color, otherwise the fog keeps the custom color at all times. -->
{
