...
<#assign biomes = w.filterBrokenReferences(data.biomesInDimension)>
<#assign caveBiomes = w.filterBrokenReferences(data.biomesInDimensionCaves)>
...
<#macro multiNoiseSource>
{
    ...
    "biomes": [
	  ...
	  <#if caveBiomes?has_content>,</#if>
	  <#list caveBiomes as biome>
      {
          "biome": "${biome}",
          "parameters":
		  <#if biome.getUnmappedValue().startsWith("CUSTOM:")>
