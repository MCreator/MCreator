	...
	public static void registerToDimension(MinecraftServer server, LevelStem levelStem) {
		...
		if(chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
			...
			if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
				List<SurfaceRules.RuleSource> surfaceRules = sequenceRuleSource.sequence();

                <#-- TODO: implement these, will need to add into surfaceRules
                ${mappedBlockToBlockStateCode(data.groundBlock)}
                ${mappedBlockToBlockStateCode(data.undergroundBlock)}
                -->

				noiseGeneratorSettings.surfaceRule = SurfaceRules.sequence(surfaceRules);
			}
		}
		*/
	}
