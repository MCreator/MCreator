...
if (${input$entity} instanceof LivingEntity _entity && !_entity.level.isClientSide() && _entity.getServer() != null) {
	...
	for (ItemStack itemstackiterator : _entity.getServer().getLootTables().get(${toResourceLocation(input$location)}).getRandomItems(new LootContext.Builder((ServerLevel) _entity.level)
			.withParameter(LootContextParams.THIS_ENTITY, _entity).withOptionalParameter(LootContextParams.LAST_DAMAGE_PLAYER, _entity.getLastHurtByMob() instanceof Player _player ?  _player : null)
			.withParameter(LootContextParams.DAMAGE_SOURCE, _ds).withOptionalParameter(LootContextParams.KILLER_ENTITY, _ds.getEntity()).withOptionalParameter(LootContextParams.DIRECT_KILLER_ENTITY, _ds.getDirectEntity())
