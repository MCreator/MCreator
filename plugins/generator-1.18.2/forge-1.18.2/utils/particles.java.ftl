...
for(int l=0;l< ${amount}; ++l) {
	double x0 = x + (random.nextFloat() - 0.5) * ${radius}D * 20;
	double y0 = y + 1.2 + (random.nextFloat() - 0.5) * ${radius}D;
	double z0 = z + (random.nextFloat() - 0.5) * ${radius}D * 20;
	world.addParticle(${particleObj.toString()}, x0, y0, z0-0.5, 0, 0, 0);
}
</#macro>
...
for (int l = 0; l < ${amount}; ++l) {
	double x0 = x + (random.nextFloat() - 0.5) * ${radius}D;
	double y0 = y + 1.2 + (random.nextFloat() - 0.5) * ${radius}D;
	double z0 = z + (random.nextFloat() - 0.5) * ${radius}D;
