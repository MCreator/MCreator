		...
		@SubscribeEvent public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeSpecialEffects effects = new BiomeSpecialEffects.Builder()
                        ...
                        .backgroundMusic(Musics.createGameMusic((SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("${data.music}"))))
                        </#if>
				...
				<#if (data.treesPerChunk > 0)>
					...
					biomeGenerationSettings.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
							Feature.TREE.withConfiguration((new TreeConfiguration.TreeConfigurationBuilder(
								...
								new MegaPineFoliagePlacer(ConstantInt.of(0), ConstantInt.of(0), FeatureSpread.func_242253_a(3, 4)),
					...
					biomeGenerationSettings.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
							Feature.TREE.withConfiguration((new TreeConfiguration.TreeConfigurationBuilder(
								new SimpleStateProvider(${ct?then(mappedBlockToBlockStateCode(data.treeStem), "Blocks.SPRUCE_LOG.getDefaultState()")}),
								new SimpleStateProvider(${ct?then(mappedBlockToBlockStateCode(data.treeBranch), "Blocks.SPRUCE_LEAVES.getDefaultState()")}),
								new MegaPineFoliagePlacer(ConstantInt.of(0), ConstantInt.of(0), FeatureSpread.func_242253_a(13, 4)),
								new GiantTrunkPlacer(${ct?then(data.minHeight, 13)}, 2, 14),
								new TwoLayerFeature(1, 1, 2)))
								<#if (data.treeVines?has_content && !data.treeVines.isEmpty()) || (data.treeFruits?has_content && !data.treeFruits.isEmpty())>
									<@vinesAndCocoa/>
