	...
	<#list entities as entity>
		...
		<#else>
			...
			public static final EntityType<${entity.getModElement().getName()}Entity>_PROJECTILE ${entity.getModElement().getRegistryNameUpper()} =
					register("entitybullet${entity.getModElement().getRegistryName()}",
							EntityType.Builder.<${entity.getModElement().getName()}ProjectileEntity>create(${entity.getModElement().getName()}ProjectileEntity::new, MobCategory.MISC)
							.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(1)
							.setCustomClientFactory(${entity.getModElement().getName()}ProjectileEntity::new).sized(0.5f, 0.5f));
			</#if>
		</#if>
    </#list>
