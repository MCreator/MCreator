try {
	if (${input$entity} instanceof ServerPlayerEntity) {
		...
		for (ItemStack itemstackiterator : _ent.server.getLootTableManager().getLootTableFromLocation(new ResourceLocation(${input$lootTable})).generate((new LootContext.Builder(_ent.getServerWorld())).withParameter(LootParameters.THIS_ENTITY, _ent)
