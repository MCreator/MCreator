	...
	if (template != null) {
		template.placeInWorld(_serverworld,
			...
			new StructurePlaceSettings()
				.setRotation(Rotation.<#if field$rotation?string != "RANDOM">${field$rotation!'NONE'}<#else>values()[_serverworld.random.nextInt(3)]</#if>)
				.setMirror(Mirror.<#if field$mirror?string != "RANDOM">${field$mirror!'NONE'}<#else>values()[_serverworld.random.nextInt(2)]</#if>)
