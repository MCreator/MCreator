...
<#include "../mcitems.ftl">
{
    ...
    <#if data.recipeShapeless>
        "ingredients": [
          ...
          <#list data.recipeSlots as element>
              <#if !element.isEmpty()>
                  <#assign ingredients += "\"${mappedMCItemToRegistryName(element, true)}\",">
              </#if>
          </#list>
    ...
    <#else>
        ...
        <#list patternKeys.keySet() as item>
            "${patternKeys.get(item)}": "${mappedMCItemToRegistryName(item, true)}"<#sep>,
        </#list>
        },
    </#if>
    "result": {
        ${mappedMCItemToItemObjectJSON(data.recipeReturnStack, "id")},
