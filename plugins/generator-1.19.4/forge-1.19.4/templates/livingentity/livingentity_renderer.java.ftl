		...
		this.addLayer(new RenderLayer<${name}Entity, ${model}>(this) {
			...
			@Override public void render(PoseStack poseStack, MultiBufferSource bufferSource, int light,
				...
				<#if layer.model != "Default">
					...
					model.renderToBuffer(poseStack, vertexConsumer, 15728640, LivingEntityRenderer.getOverlayCoords(entity, 0f), 1, 1, 1, 1);
				<#else>
					this.getParentModel().renderToBuffer(poseStack, vertexConsumer, 15728640, LivingEntityRenderer.getOverlayCoords(entity, 0f), 1, 1, 1, 1);
				</#if>
